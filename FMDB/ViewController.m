...
#pragma mark - *************** 所有插入操作
- (void)insertMethod:(ProductionView *)proViews db:(LXSFMDB *)db model:(Person *)person array:(NSMutableArray *)mArr
{
    [proViews insertMethod1:^{
        //插入一条数据
        [db lxs_insertTable:@"user" dicOrModel:person];
        [proViews reloadData]; //刷新tableview
    }];

    [proViews insertMethod2:^{
        //插入一组数据, 数据多建议使用事务插入, 效率很高
        [db lxs_insertTable:@"user" dicOrModelArray:mArr];
        [proViews reloadData]; //刷新tableview
    }];
...
#pragma mark - *************** 所有删除操作
- (void)deleteMethod:(ProductionView *)proViews db:(LXSFMDB *)db model:(Person *)person array:(NSMutableArray *)mArr
{
    [proViews deleteMethod1:^{
        //删除最后一条数据
        [db lxs_deleteTable:@"user" whereFormat:@"WHERE rowid = (SELECT max(rowid) FROM user)"];
        [proViews reloadData]; //刷新tableview
    }];

    [proViews deleteMethod2:^{
        //删除全部数据
        [db lxs_deleteAllDataFromTable:@"user"];
        [proViews reloadData]; //刷新tableView;
    }];
...
#pragma mark - *************** 所有更新操作
- (void)updateMethod:(ProductionView *)proViews db:(LXSFMDB *)db model:(Person *)person array:(NSMutableArray *)mArr
{
    [proViews deleteMethod1:^{
        //更新最后一条数据 name=testName , dicOrModel的参数也可以是name为testName的person
        [db lxs_updateTable:@"user" dicOrModel:@{@"name":@"testName"} whereFormat:@"WHERE rowid = (SELECT max(rowid) FROM user)"];
        [proViews reloadData]; //刷新tableview
    }];

    [proViews deleteMethod2:^{
        //把表中所有的name改成godlike
        [db lxs_updateTable:@"user" dicOrModel:@{@"name":@"godlike"} whereFormat:nil];
        [proViews reloadData]; //刷新tableView;
    }];
    ...
    [proViews deleteMethod3:^{
        ...
        [db lxs_inDatabase:^{
            [db lxs_updateTable:@"user" dicOrModel:@{@"name":@"safeName"} whereFormat:@"WHERE rowid = (SELECT max(rowid) FROM user)"];
            [proViews reloadData]; //刷新tableview
        }];
    }];
    ...
    [proViews deleteMethod4:^{
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [db lxs_inDatabase:^{
                [db lxs_updateTable:@"user" dicOrModel:@{@"name":@"asyncName"} whereFormat:@"WHERE rowid = (SELECT max(rowid) FROM user)"];
            }];
            dispatch_sync(dispatch_get_main_queue(), ^{
                [proViews reloadData]; //刷新tableview
            });
...
#pragma mark - *************** 所有查找操作
- (void)lookupMethod:(ProductionView *)proViews db:(LXSFMDB *)db model:(Person *)person array:(NSMutableArray *)mArr
{
    [proViews lookupMethod1:^{
        ////查找name=cleanmonkey的数据
        NSArray *personArr = [db lxs_lookupTable:@"user" dicOrModel:[Person class] whereFormat:@"where name = 'cleanmonkey'"];
    ...
    [proViews lookupMethod2:^{
        //查找表中所有数据
        NSArray *personArr = [db lxs_lookupTable:@"user" dicOrModel:[Person class] whereFormat:nil];
...
#pragma mark - *************** 事务操作
- (void)transactionMethod:(ProductionView *)proViews db:(LXSFMDB *)db model:(Person *)person array:(NSArray *)mArr
{
    [proViews transactionMethod1:^{
        //用事务插入1000条数据, 数据量多的话用事务插入会很快, 就像生产一件零件就送走和生产一堆零件再送走的效率问题
        [db lxs_inTransaction:^(BOOL *rollback) {
            for (int i = 0; i < 1000; i++) {
                BOOL flag = [db lxs_insertTable:@"user" dicOrModel:person];
                if (flag == NO) {
                    *rollback = YES; //回滚操作
